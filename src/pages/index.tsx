...
<span key={stat-1-${idx}} className="hover:text-blue-600 transition-colors"&gt;{stat}&lt;/span&gt; ))} &lt;/div&gt; &lt;div className="flex gap-16 shrink-0 justify-around min-w-full"&gt; {tickerStats.map((stat, idx) =&gt; ( &lt;span key={stat-2-${idx}} className="hover:text-blue-600 transition-colors">{stat}</span>
))}
</div>
</div>
</div>
...
<span dangerouslySetInnerHTML={{ __html: &lt;style&gt; @keyframes gpnrMarquee { 0% { transform: translateX(0%); } 100% { transform: translateX(-100%); } } .compliance-marquee { animation: gpnrMarquee 40s linear infinite !important; } .compliance-marquee:active, .compliance-marquee:hover { animation-play-state: paused !important; } &lt;/style&gt;}} />
</main>
);
}
